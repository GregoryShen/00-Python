...
之前谈到的是Python的单继承,还有一种继承方式是多继承,Python在原生语法里是支持多继承的,多继承和单继承的区别主要在于多继承能够从好几个类里继承属性和方法,我们首先把

```python
class BackendProgrammer(Programmer):
    super(BackendProgram)
...
在概念上并不完全一样,实际上不完全是重写的语法,但它的机制上
